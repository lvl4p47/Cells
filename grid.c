...
void Grid_Set(int16_t x, int16_t y, uint16_t id)
{
    ...
    if(id == 0)
    {
        if(temp_id != 0 && temp_id <= MAX_ORGANISMS)
        {
    ...
    if(id <= MAX_ORGANISMS && population[id].alive && population[id].material > 0)
    {
        ...
        if(temp_id <= MAX_ORGANISMS && temp_id != id)
        {
            if(temp_id == MAX_ORGANISMS)
            {
               ...
               || population[temp_id].alive == 0))
            {
                ...
                else if(grid_array[y1][x1].type == 1)  // Ядро
                {
                    ...
                    population[id].material += grid_array[y1][x1].mat - 1;
                    grid_array[y1][x1].mat = 1;
                    population[id].lifetime += population[temp_id].lifetime / 4;
...
void Grid_Update()
{
    ...
    // printf("\nvol_mat: %5d org_mat: %5d grid_mat: %5d total_mat: %5d", vol_mat, org_mat, grid_mat, total_mat);
    ...
    for(int id0 = 1; id0 < MAX_ORGANISMS; id0++)
    {
        ...
        if(population[id1].alive)
        {
            ...
            || new_min_y > new_max_y) 
            {
                // printf("\nreason of death: no volume");
                // printf("\nnew_min_x: %4d new_min_y: %4d new_max_x: %4d, new_max_y: %4d", new_min_x, new_min_y, new_max_x, new_max_y);
                // printf("\nnuc_x: %4d nuc_y %4d", population[id1].nuc_x, population[id1].nuc_y);
                Organism_Quit(id1);
                continue;
            }
            else
                            ...
                            else if(lifetime != 0)
                            {
                                if(Grid_Get(j, i)->strength != 0)
                                {
                                    ...
                                    if(w_total != 0)
                                    {
                                        ...
                                        {
                                            if(ax > ay) Grid_Signal(j + sx, i, vx, vy, strength);
                                            else Grid_Signal(j, i + sy, vx, vy, strength);
                                        }
                                    }
    ...
    // printf("free_top: %d", free_top);
    
    
    // printf("\n%d ", population[19].multiply);
    // printf("\n");
    // for(int i = 0; i < MAX_ORGANISMS; i++)
    // {
    //     printf("%d ", free_stack[i]);
...
void Genome_Hash(uint16_t id)
{
    ...
    uint16_t hash = 0;
    for (int i = 0; i < GENOME_SIZE; i++) {
        hash += population[id].genome[i] * (i + 1);
    }
...
void Best_Genome_Spread()
{
    ...
    while(current_best >= MAX_ORGANISMS * (re_frac - 1) / re_frac)
    {
        ...
        {
            Genome_Copy(current_best, org, 10);
...
void Organism_Quit(uint16_t id)
{
    if(debug) fprintf(stderr, "\nOrganism_Quit");
    // printf("\norganism_quit ");
    if (id == 0 || id >= MAX_ORGANISMS) return;
    if (population[id].alive == 0) return;
    // printf("%d", id);
    Grid_Signal(population[id].nuc_x, population[id].nuc_y, 0, 0, 0);
    if(population[id].material > 0)
    {
        // printf("\norg_quit id: %d", id);
        Grid_Set(population[id].nuc_x, population[id].nuc_y, id);
    ...
    population[id].pacifism_treshold = 1;
    population[id].life_wave_timer = 0;
    // population[id].min_x = population[id].nuc_x;
    // population[id].min_y = population[id].nuc_y;
    // population[id].max_x = population[id].nuc_x;
    // population[id].max_y = population[id].nuc_y;
}
...
uint16_t Is_Membrane(int16_t x, int16_t y)
{
    ...
    for(int dy = -1; dy < 2; dy++)
    {
        for(int dx = -1; dx < 2; dx++)
        {
            ...
            {
                counter++;
                if(id1 == MAX_ORGANISMS) 
                {
                    other += Grid_Get(x1 + dx, y1 + dy)->mat * 10;
                }
                else if(id1 != 0)
                {
                    other += Grid_Get(x1 + dx, y1 + dy)->mat;
                }
            }
        }
    }
    ...
    if(population[id].alive && other > population[id].target_str)
    {
        population[id].target_str = other;
    ...
    if(strength > 0)
    {
        ...
        for(int dy = -1; dy < 2; dy++)
        {
            for(int dx = -1; dx < 2; dx++)
            {
                ...
                uint8_t penetrate = (
                ((abs(vx) + abs(vy)) * (rand() % max(population[id].volume, 1 ))
                 > max(population[id_to].volume, 1 ))
                && id_to != 0);
    ...
    for(int id = 0; id < MAX_ORGANISMS; id++)
    {
        ...
        if(population[i].alive)
        {
            ...
            if(population[i].life_wave_timer >= 127)
            {
                ...
                grid_array[y][x].life_wave_str = population[i].volume;
            }
            ...
            case ACCEL:
                ...
                if(abs(population[i].vx) > 63)
                    population[i].vx = (population[i].vx > 0) ? 127 : -128;
                else 
                    population[i].vx *= 2;
                
                if(abs(population[i].vy) > 63)
                    population[i].vy = (population[i].vy > 0) ? 127 : -128;
                else 
                    population[i].vy *= 2;
                if(print) printf("\n ACCEL ");
                break;
            ...
            case GROW:
                ...
                && population[i].vy == 0)
                {
                    population[i].vx = rand() % 256 - 127;
                    population[i].vy = rand() % 256 - 127;
                }
                if(print) printf("\n GROW ");
                break;
            case SHRINK:
            ...
            case STR_ONE:
                ...
                break;
            case STR_MAX:
                population[i].strength = population[i].volume;
                if(print) printf("\n STR_MAX ");
                break;
            case GOTO_0:
                population[i].gp = 0;
            ...
            case SKIP:
                ...
                break;
            case CHECK_OTHER:
                dp = (population[i].target_str > 0);
                population[i].gp = mod(population[i].gp + dp, GENOME_SIZE);
                if(print) printf("\n CHECK_OTHER ");
                break;
            case CHECK_TARGET_DX:
                if(population[i].target_str)
                {
                    dp = sign(population[i].target_dx) + 2;
                }
                if(print) printf("\n CHECK_TARGET_DX ");
                break;
            case CHECK_TARGET_DY:
                if(population[i].target_str)
                {
                    dp = sign(population[i].target_dy) + 2;
                }
                if(print) printf("\n CHECK_TARGET_DY ");
                break;
            case CHECK_PAIN_DX:
                if(population[i].pain_str)
                {
                    dp = sign(population[i].pain_dx) + 2;
                }
                if(print) printf("\n CHECK_PAIN_DX ");
                break;
            case CHECK_PAIN_DY:
                if(population[i].pain_str)
                {
                    dp = sign(population[i].pain_dy) + 2;
                }
                if(print) printf("\n CHECK_PAIN_DY ");
                break;
            case CHECK_MAT:
                dp = (population[i].material > 0) + 1;
                if(print) printf("\n CHECK_MAT ");
                break;
            case CHECK_MULT:
                dp = (population[i].multiply > 0) + 1;
                if(print) printf("\n CHECK_MULT ");
                break;
            case MULTIPLY:
                if(population[i].material > 1 * population[i].min_mat) 
                {
                    multiply = 1;
                    ...
                    && population[i].vy == 0)
                    {
                        population[i].vx = rand() % 255 - 127;
                        population[i].vy = rand() % 255 - 127;
                    }
                }
                if(print) printf("\n MULTIPLY ");
                break;
            ...
            case SET_STATE_2:
                ...
                break;
            case IF_STATE_0:
                dp = (population[i].state == 0) + 1;
                break;
            case IF_STATE_1:
                dp = (population[i].state == 1) + 1;
                break;
            case IF_STATE_2:
                dp = (population[i].state == 2) + 1;
                break;
            case MUTATE_POS:
            ...
            uint8_t move_prob = (ax + ay) / 2;  // Средняя скорость, макс 127
            if(rand() % 128 < move_prob) {
            ...
            if(grow) 
            {
                ...
                Grid_Signal(population[i].nuc_x, 
                population[i].nuc_y, 
                population[i].vx, 
                population[i].vy, 
                min(population[i].strength, population[i].volume));
                if(population[i].material == 0) shrink = 1;
            }
            
            if(shrink) Grid_Signal(population[i].nuc_x, 
            population[i].nuc_y, 
            -population[i].vx, 
            -population[i].vy, 
            -min(population[i].strength, population[i].volume) - 1);
                ...
                if(Grid_Get(population[i].nuc_x + deltax, population[i].nuc_y + deltay)->id == 0)
                {
                    if(population[i].material > 1 * population[i].min_mat)
                    {
                        ...
                        if(child_id != 0) {
                            ...
                            population[i].lifetime = max(population[i].lifetime - lifetime / 4, 0);
                            // printf("multiply success mat: %d", population[child_id].material);
                        } else {
                            population[i].material += population[i].min_mat;
            ...
            if(multiply && population[i].material <= 1 * population[i].min_mat)
            {
                multiply = 0;
                population[i].multiply = 0;
            }
...
void Repopulate()
{
    ...
    for(int i = 0; i < MAX_ORGANISMS * 10; i++)
    {
        x = rand() % grid_width;
        y = rand() % grid_height;
